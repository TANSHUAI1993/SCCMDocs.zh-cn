---
title: "为没用使用 System Center Configuration Manager 管理的 Windows 8.1 和 Windows 10 设备创建配置项目 | System Center Configuration Manager"
...
|**密码复杂性** - **密码中所需的复杂字符集数量**|如果选择“强”密码，使用此设置配置所需的复杂字符集数量。 对于强密码，至少应设置为 **3**，这意味着同时需要字母和数字。 如果要强制采用额外需要特殊字符（如 **(%$**）的密码，请选择 **4**。|
|**将密码恢复 PIN 发送到 Exchange Server**|设置为“启用”或“禁用”。|  
...
|**设置同步**|允许设备之间的设置同步。|  
|**凭据同步**|允许设备之间的凭据同步。|  
|**Microsoft 帐户**|允许在设备上使用 Microsoft 帐户。|  
