---
title: "如何为未使用 System Center Configuration Manager 客户端管理的 Windows Phone 设备创建配置项目 | System Center Configuration Manager"
ms.custom: na
ms.date: 11/18/2016
...
使用 System Center Configuration Manager **Windows Phone** 配置项目，为已在 Microsoft Intune 中注册或通过 Configuration Manager 本地管理的 Windows Phone 的设备管理设置。  
...
7.  在“支持的平台”页上，选择评估配置项目的特定 Windows Phone 平台。  

8.  在“设备设置”页面上，选择要配置的设置组。 请参阅本主题中的 [Windows Phone 配置项目设置参考](/sccm/compliance/deploy-use/create-configuration-items-for-windows-phone-devices-managed-without-the-client#windows-phone-configuration-item-settings-reference)以了解详细信息，然后单击“下一步”。  
...
|**锁定设备前的空闲时间**|指定设备自动锁定屏幕前，必须保持空闲的时间。|  
...
|设置|详细信息|  
|-------------|-------------|  
|**允许使用 Web 浏览器**|用户可以更改默认 Internet 浏览器。|  
...
|**允许使用 USB 连接**|控制设备是否可通过 USB 连接访问外部存储设备。|
...
2.  在“无线网络连接”对话框中，在移动设备上指定有关要设置的无线连接的以下信息，然后单击“确定”：
...
### <a name="windows-phone-allowed-and-blocked-apps-list-windows-phone-81-only"></a>Windows Phone 允许的和阻止的应用列表（仅限 Windows Phone 8.1）  
