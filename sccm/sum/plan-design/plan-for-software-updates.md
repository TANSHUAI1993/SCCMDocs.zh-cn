...
安装 WSUS 时，需要提供内容目录路径。 WSUS 内容目录主要用于存储客户端在扫描过程中所需的 Microsoft 软件许可条款文件。 WSUS 内容目录 Configuration Manager 不应与 Configuration Manager 软件部署包的内容源目录重叠。 与 WSUS 内容目录和 Configuration Manager 包源重叠会导致从 WSUS 内容目录中删除错误的文件。
