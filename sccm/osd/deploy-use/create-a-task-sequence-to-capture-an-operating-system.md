...
5. 在“任务序列信息”  页上，指定以下设置，然后单击“下一步” 。  
   ...
   -   **描述**：指定任务序列所执行的任务的描述，例如任务序列创建的操作系统的描述。  
...
6. 在“安装 Windows”  页上，指定以下设置，然后单击“下一步” 。  
   ...
   -   指定如何处理在部署操作系统时使用的管理员帐户。  
       ...
       -   **随机生成本地管理员密码并在所有支持的平台上禁用帐户**：指定是否让 Configuration Manager 为本地管理员帐户创建随机密码，并在部署操作系统时禁用帐户。  

       -   **启用帐户并指定本地管理员密码**：指定是否为部署操作系统的所有计算机上的本地管理员帐户使用同一密码。  
