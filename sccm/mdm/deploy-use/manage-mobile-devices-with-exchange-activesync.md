...
>  在安装 Exchange Server 连接器之前，请确认 Configuration Manager 支持你所使用的 Microsoft Exchange 的版本。 有关详细信息，请参阅 [System Center Configuration Manager 站点和客户端支持的操作系统](/sccm/core/plan-design/configs/supported-operating-systems-for-site-system-servers)中的“Exchange Server 连接器”。  
 ...
 在使用 Exchange Server 连接器时，可通过在 Configuration Manager 中配置的设置来管理移动设备，而不是通过默认 Exchange ActiveSync 邮箱策略进行管理。 定义要在以下组设置中使用的设置：**常规**、**密码**、**电子邮件管理**、**安全性**和**应用程序**。 例如，在“密码” 组设置中，你可以配置移动设备是否需要密码、最小密码长度、密码复杂性以及是否允许密码恢复。  
...
> [!TIP]  
>  如果通过使用 Exchange Server 连接器来管理移动设备，并且移动设备已转让给另一用户，请从 Configuration Manager 控制台中删除该移动设备，之后移动设备的新所有者才能在此传输移动设备。  

## <a name="required-security-permissions"></a>所需的安全权限  
 你必须具有以下安全权限才能配置 Exchange Server 连接器：  

- 添加、修改和删除 Exchange Server 连接器：“站点”对象的“修改”权限。  

- 配置移动设备设置：**站点**对象的 **"modifyconnectorpolicy"** 权限。  
...
1.  决定哪个帐户将连接到 Exchange 客户端访问服务器来管理移动设备。 该帐户可以是站点服务器的计算机帐户或 Windows 用户帐户。 然后，配置此帐户以运行以下 Exchange Server cmdlet：  
    ...
    > [!NOTE]  
    >  以下 Exchange Server 管理角色包括这些 cmdlet："收件人管理"、"仅查看组织管理" 和 "服务器管理"。 有关 Microsoft Exchange Server 2010 中的管理角色组的详细信息，请参阅 [了解管理角色组](https://go.microsoft.com/fwlink/p/?LinkId=212914)。  
