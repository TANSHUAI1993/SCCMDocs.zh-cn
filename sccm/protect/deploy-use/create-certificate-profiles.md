...
-   **个人信息交换- PKCS #12 (PFX) 设置 - 导入**：选择此选项将导入 PFX 证书。 若要了解有关 PFX 证书创建的详细信息，请参阅[导入 PFX 证书配置文件](/sccm/mdm/deploy-use/import-pfx-certificate-profiles.md)。

-   个人信息交换 – PKCS #12 (PFX) 设置 – 导入 **：选择此项可处理 PFX 证书使用的证书颁发机构。 若要了解有关 PFX 证书创建的详细信息，请参阅 [Create PFX certificate profiles](/sccm/mdm/deploy-use/create-pfx-certificate-profiles.md)（创建 PFX 证书配置文件）。
...
3. 在“创建证书配置文件向导”的“证书属性”  页上，指定下列信息：  
   ...
   -   **证书模板名称**：单击“浏览”以选择网络设备注册服务配置为使用并且已添加到颁发 CA 的证书模板的名称。 若要成功浏览到证书模板，用于运行 System Center Configuration Manager 控制台的用户帐户必须对证书模板具有“读取”权限。 或者，如果你无法使用“浏览” ，请键入证书模板的名称。  
   ...
   要查找证书模板名称，请浏览到下列项：HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Cryptography\MSCEP。 你将看到作为 **EncryptionTemplate**、 **GeneralPurposeTemplate**和 **SignatureTemplate**的值而列出的证书模板。 默认情况下，所有三个证书模板的值为“IPSECIntermediateOffline” ，映射为模板显示名称“IPSec (脱机请求)” 。  
