...
ms.locfileid: "55972048"
---
## <a name="bkmk_odfb"></a> 将 Windows 已知文件夹重定向到 OneDrive
<!--3556021-->
...
4. 在“设置”页上：
    ...
    2. 选择以下选项之一将已知文件夹移动到 OneDrive：  
        ...
        - **提示用户将 Windows 已知文件夹移动到 OneDrive**：使用此选项，用户会看到一个向导，以移动其文件。 如果用户选择推迟或拒绝移动其文件夹，则 OneDrive 将定期提醒他们。  

        - **以无提示方式将 Windows 已知文件夹移动到 OneDrive**：将此策略应用于设备时，OneDrive 客户端将自动将已知文件夹重定向到 OneDrive for Business。  

            - **重定向文件夹后向用户显示通知**：如果启用此选项，则 OneDrive 客户端将在它移动用户的文件夹后通知用户。  
