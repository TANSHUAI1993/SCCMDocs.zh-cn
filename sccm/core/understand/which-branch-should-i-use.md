...
*适用范围：System Center Configuration Manager (Current Branch)、(Long-Term Servicing Branch)、(Technical Preview)*
...
此经过许可的分支可用于需要获取最新功能的选项的生产环境。 如果拥有以下项之一，应使用该分支：System Center 数据中心、System Center Standard、System Center Configuration Manager 或等效订阅权限。  有关软件保障协议和许可选项的详细信息，请参阅 [System Center Configuration Manager 的许可和分支](learn-more-editions.md)。


>  [!TIP]
> Current Branch 也可作为不需要许可证的评估版安装。 评估版可使用 180 天，并支持升级到 Current Branch 的许可版。

Current Branch 一年更新几次，更新时提供新的功能。 每个更新版本在其发布后一 (1) 年内受支持。 必须在 1 年期限到期之前或到期日当天升级到 Current Branch 的较新版本。 较新版本的更新以控制台内更新的形式提供。

若要将 Current Branch 作为新站点或者作为从*System Center 2012 Configuration Manager Service Pack 2* 或*System Center 2012 R2 Configuration Manager Service Pack 1* 的升级对其进行安装，需要使用 System Center Configuration Manager 的[基线介质](/sccm/core/servers/manage/updates#baseline-and-update-versions)，该基线介质以 DVD 形式随附在 System Cener 2016 中，或者作为 System Center Configuration Manager 的一部分单独发行。 对此介质的访问权取决于 System Center Configuration Manager 的许可方式。

也可以使用基线介质来安装 Current Branch 的评估版新站点。 如果要仅安装评估版，可以从 [TechNet 评估中心](https://www.microsoft.com/evalcenter/evaluate-system-center-configuration-manager-and-endpoint-protection)网站获取软件。

>  [!NOTE]
> 仅使用基线介质安装新的 Configuration Manager 层次结构的站点。 如果以前安装了基准版本（如版本 1511），使用控制台中更新将站点更新到新版本（如 1606）。
>
> 使用控制台中更新更新后的站点与使用基线介质安装新站点的效果一样。
>
...
- 支持评估安装，以后可以升级到完全许可的安装。

Current Branch 的初始版本是版本 1511。 后续更新包括版本 1602 和 1606 等。 每个版本在一年内受支持，Microsoft 建议在最新版本发布后立即更新。 较旧版本最多可使用一年，之后需要更新到较新版本，也可跳过更新，直接安装最新的可用版本。 由于每个版本是累积更新，因此如果跳过更新并安装最新版本，仍可获取来自以前版本的所有功能和改进。
...
##  <a name="long-term-servicing-branch-ltsb-of-system-center-configuration"></a>System Center Configuration Manager 的 Long-Term Servicing Branch (LTSB)
这是可用于生产的许可的分支，适合正在使用 Current Branch 且允许其 Configuration Manager 软件保障 (SA) 或等效订阅权限在 2016 年 10 月 1 日之后过期的 Configuration Manager 客户。  有关软件保障和许可选项的信息，请参阅 [System Center Configuration Manager 的许可和分支](learn-more-editions.md)。
...
- 如果有当前 SA 协议或 Configuration Manager 等效权限，支持升级（转换）到 Current Branch。
...
- 支持有限的服务器和客户端操作系统以及相关技术（如 SQL Server 版本）。  有关此分支支持功能的详细信息，请参阅 [Long-Term Servicing Branch 的支持配置](supported-configurations-for-ltsb.md)。
- 不接收新功能的更新。
...
-   不能将 Current Branch 的评估版升级到 LTSB 安装。
...
-  接收控制台中更新，将安装更新到最新的 Technical Preview 版本。
-  包括正在开发的新功能，开发人员希望获得你对这些新功能的反馈。
