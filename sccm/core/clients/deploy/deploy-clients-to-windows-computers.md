...
> [!IMPORTANT]  
> 只有满足所有先决条件，客户端请求才会成功。 有关详细信息，请参阅[安装方法依赖项](/sccm/core/clients/deploy/prerequisites-for-deploying-clients-to-windows-computers#installation-method-dependencies)。
...
安装 Configuration Manager 客户端后，设备不会从 Intune 中注销。 它们可以同时使用 Configuration Manager 客户端和 MDM 注册。 有关详细信息，请参阅[共同管理概述](/sccm/comanage/overview)。  
...
1. 在 Intune 中，[添加 Windows 业务线应用](https://docs.microsoft.com/intune/lob-apps-windows)，其中包含 Configuration Manager 客户端安装文件 CCMSetup.msi  。 此文件位于站点服务器的 Configuration Manager 安装目录的 `\bin\i386` 文件夹中。  
...
## <a name="BKMK_Provision"></a> 预配客户端安装属性
