...
###  <a name="BKMK_ToInstallLnUClinent"></a>在 Linux 和 UNIX 服务器上安装 Configuration Manager 客户端  
...
-   `-UsePKICert <parameter>`  
     ...
     可选。 公钥证书标准 (PKCS #12) 格式指定完整路径和文件名为 X.509 PKI 证书的名称。 此证书用于客户端身份验证。 如果在安装过程中未指定证书，则你需要添加或更改证书，请使用 **certutil** 实用工具。 有关详细信息，请参阅[如何管理适用于 Linux 和 UNIX 的客户端上的证书](/sccm/core/clients/manage/manage-clients-for-linux-and-unix-servers#BKMK_ManageLinuxCerts)。  
...
###  <a name="BKMK_UninstallLnUClient"></a>从 Linux 和 UNIX 服务器卸载客户端  
