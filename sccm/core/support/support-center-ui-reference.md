...
- **注册状态**：指示客户端是否注册到 Configuration Manager  
...
- **位置服务**  
    ...
    - **刷新内容位置**：刷新任何当前内容下载使用的分发点。  

    - **刷新管理点**：更新客户端使用的管理点内部列表。  
...
- **清除所有条目**：清除窗口中显示的所有日志文件条目。 支持中心将来不会显示这些条目。  
- **关闭所有日志**：关闭打开的所有日志文件  
...
- **实时更新**：监视当前打开的日志文件的更改。 打开多个日志文件时，此功能不起作用。 默认情况下会启用此选项。  

- **自动滚动**：如果你还选择了“实时更新”选项，此选项会自动滚动日志视图，以显示新添加的条目。 打开多个日志文件时，此功能不起作用。 默认情况下会启用此选项。  
...
- **日志文本换行**：使较长的多行消息换行以放入单列。 此行为使这些消息更容易阅读。 默认情况下会启用此选项。  
