...
**不可从当前边界组访问内容时：**  
在不可从当前边界组中的有效内容源访问客户端请求的内容时，客户端使用即时回退来搜寻邻居边界组的分发点中的内容：   
- 即时回退发生于邻居边界组或配置为最小回退时间的组。 如果邻居边界组没有更短的回退时间时，这可能包括默认站点边界组。
- 即时回退到第一组邻居边界组后，会根据其他边界组配置的回退时间，回退到这些组。

如果按需分发内容，但客户端请求时不可访问该内容，则开始将内容传输到当前边界中的分发点这一过程。 但是，因为此时不可访问该内容，所以客户端即时回退到回退时间最短的邻居边界组。 可以访问当前边界组中的内容后，其他客户端不再即时回退到邻居组。
