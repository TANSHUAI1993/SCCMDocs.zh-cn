...
-   默认情况下，安装 Configuration Manager 主站点时，检测信号已处于启用状态，并配置为按基本计划运行。 应保持检测信号发现为启用状态，因为此方法可确保设备的发现数据记录 (DDR) 保持最新。 有关检测信号发现的详细信息，请参阅[关于检测信号发现](../../../../core/servers/deploy/configure/about-discovery-methods.md#bkmk_aboutHeartbeat)  

-   服务器发现是一种自动发现方法，用于查找用作站点系统的计算机，不能对其进行配置或禁用。  

**启用任何可配置的发现方法：**  

1.  在 Configuration Manager 控制台中，依次单击“管理” > “层次结构配置”，然后单击“发现方法”。  

2.  为要在其中启用发现的站点选择发现方法。  

3.  在“主页”选项卡上的“属性”组中，单击“属性”，然后在“常规”选项卡上，选择“启用&lt;发现方法\>”复选框。  

     如果此复选框已处于选中状态，你可以通过清除复选框来禁用发现方法。  
...
##  <a name="a-namebkmkconfigadforestdisca-configure-active-directory-forest-discovery"></a><a name="BKMK_ConfigADForestDisc"></a>配置 Active Directory 林发现  
 要完成 Active Directory 林发现的配置，你必须在两个位置中配置设置：  

-   在“发现方法”  节点中，你可以启用此发现方法、设置轮询计划，并选择发现是否自动为其发现的 Active Directory 站点和子网创建边界。  

-   在“Active Directory 林”节点中，可以添加想要发现的林，启用该林中的 Active Directory 站点和子网发现，配置可启用 Configuration Manager 站点以发布其站点信息到林的设置，并且分配一个帐户用作每个林的 Active Directory 林账户。  
...
4.  在“常规”  选项卡上，选中复选框以启用发现，或者，你可以立即配置发现，并返回以在稍后启用发现。  

5.  指定为发现的位置创建站点边界的选项。  

6.  指定有关发现何时运行的计划。  

7.  完成为此站点配置 Active Directory 林发现的操作后，单击“确定”  保存配置。  

#### <a name="to-configure-a-forest-for-active-directory-forest-discovery"></a>为 Active Directory 林发现配置林  

1.  在“管理”  工作区中，单击“Active Directory 林” 。 如果 Active Directory 林发现之前已运行，你将在结果窗格中看到每个发现的林。 当 Active Directory 林发现运行时，将发现本地林和任何受信任林。 只有不受信任的林才必须手动添加。  
...
3.  如果你打算允许站点将站点数据发布到此林，请在“发布”  选项卡上完成用于发布到此林的配置。  

    > [!NOTE]  
    >  如果使站点能够发布到林，必须为 Configuration Manager 扩展该林的 Active Directory 架构，并且 Active Directory 林帐户必须对该林中的“系统”容器具有“完全控制”权限。  

4.  完成配置此林以用于 Active Directory 林发现的操作后，单击 **OK** 保存配置。  
...
##  <a name="a-namebkmkconfigaddiscgenerala-configure-active-directory-discovery-for-computers-users-or-groups"></a><a name="BKMK_ConfigADDiscGeneral"></a>为计算机、用户或组配置 Active Directory 发现  
 使用下列部分中的信息通过以下发现方法之一来配置计算机、用户或组的发现：  
...
5.  单击“添加”  以配置发现作用域，选择“组”  或“位置” ，并在“添加组” 或“添加 Active Directory 位置”  对话框中完成以下配置：  
...
8.  （可选）在“选项”  选项卡上，你可以配置选项以从发现中筛选出或排除过期计算机记录，并发现通讯组的成员身份。  

    > [!NOTE]  
    >  默认情况下，Active Directory 组发现只会发现安全组的成员身份。  

9. 完成为此站点配置 Active Directory 组发现的操作后，单击“确定”  保存配置。  
...
4.  在“常规”  选项卡上，选中复选框以启用发现，或者，你可以立即配置发现，然后返回以在稍后启用发现。  

5.  单击“新建”图标![新建图标](media/Disc_new_Icon.gif)以指定新的 Active Directory 容器，然后在“Active Directory 容器”对话框中，完成以下配置：  

    1.  指定要搜索的一个或多个位置。  

    2.  对于每个位置，指定修改搜索行为的选项。  
    ...
    4.  单击“确定”  保存 Active Directory 容器配置。  

6.  在“轮询计划”选项卡上，配置完整发现轮询计划和增量发现。  

7.  （可选）在“Active Directory 属性”  选项卡上，你可以为要发现的计算机配置其他 Active Directory 属性。 还会列出默认对象属性。  

8.  （可选）在“选项”  选项卡上，你可以配置选项以从发现中筛选出或排除过期计算机记录。  

9. 完成为此站点配置 Active Directory 系统发现的操作后，单击“确定”保存配置。  
...
8.  完成为此站点配置 Active Directory 用户发现的操作后，单击“确定”保存配置。  

##  <a name="a-namebkmkconfighbdisca-configure-heartbeat-discovery"></a><a name="BKMK_ConfigHBDisc"></a>配置检测信号发现  
 默认情况下，检测信号发现在安装 Configuration Manager 主站点时启用。 因此，若不想使用每隔 7 天的默认设置，只需配置有关客户端将检测信号发现数据记录 (DDR) 发送到管理点的频率的计划。  
...
4.  配置客户端提交检测信号发现数据记录 (DDR) 的频率，然后单击“确定”  保存配置。  
...
有关详细信息，请参阅[关于网络发现](../../../../core/servers/deploy/configure/about-discovery-methods.md#bkmk_aboutNetwork)部分。  

 下列部分提供有关网络发现的常见配置的信息。 你可以配置其中一个或多个配置以在同一发现轮次中使用。 如果使用多个配置，你必须规划可能影响发现结果的交互。  

 例如，你可能希望发现使用特定 SNMP 共同体名称的所有 SNMP 设备。 此外，对于同一发现轮次，你可以禁用针对特定子网的发现。 当发现运行时，网络发现不会发现你已禁用的子网上具有指定共同体名称的 SNMP 设备。  
...
####  <a name="a-namebkmklimitbysubneta-limit-searches-by-using-subnets"></a><a name="BKMK_LimitBySubnet"></a>使用子网限制搜索  
 可以将网络发现配置为在发现运行期间搜索特定子网。 默认情况下，网络发现会搜索运行发现的服务器的子网。 你配置和启用的任何其他子网仅适用于简单网络管理协议 (SNMP) 和动态主机配置协议 (DHCP) 搜索选项。 当网络发现搜索域时，子网配置未对其进行限制。  
...
> [!NOTE]  
>  SNMP 共同体名称类似于密码。 网络发现只能从已指定共同体名称的 SNMP 设备中获得信息。 每个 SNMP 设备均可以拥有自己的共同体名称，但是同一个共同体名称通常会由几个设备共享。 此外，大多数 SNMP 设备都有默认的“公共” 共同体名称。 但是，某些组织将删除其设备的“公共”  共同体名称以作为安全措施。  

 如果“网络发现属性”  对话框中的“SNMP”  选项卡上显示了多个 SNMP 共同体，则网络发现按照共同体的显示顺序搜索共同体。 为了帮助最大程度降低因使用不同名称尝试连接设备所产生的网络流量，请确保最频繁使用的名称位于列表的顶部。  

> [!NOTE]  
>  除了使用 SNMP 共同体名称之外，还可以指定特定 SNMP 设备的 IP 地址或可解析名称。 你可以在“网络发现属性”  对话框中的“SNMP 设备”  选项卡上配置特定设备的 IP 地址或可解析名称。  
...
3.  在“主页”  选项卡上的“属性”  组中，单击“属性” 。  
    ...
    -   在“子网”  选项卡上，选中“搜索本地子网”  复选框。  
        ...
        > [!TIP]  
        >  如果知道构成网络的特定子网，则可以清除“搜索本地子网”复选框并使用“新建”图标![新建图标](media/Disc_new_Icon.gif)来添加要搜索的特定子网。 对于大型网络，通常最好一次仅搜索一两个子网，以最大程度降低网络带宽使用量。  
...
4.  在“计划”选项卡上，单击“新建”图标![新建图标](media/Disc_new_Icon.gif)以设置用于运行网络发现的计划。  

    > [!NOTE]  
    >  你无法将不同的发现配置分配给单独的网络发现计划。 每次运行网络发现时，它都使用当前发现配置。  

5.  单击“确定”  以接受配置。 网络发现将按计划的时间运行。  
...
5.  要将发现配置为搜索子网，请单击“子网”  选项卡，并在“子网”  选项卡上配置以下一个或多个选项：  

    -   要在运行发现的计算机的本地子网上运行发现，请选中“搜索本地子网”  复选框。  

    -   为了搜索特定子网，该子网必须在“要搜索的子网” 中列出，并且其“搜索”  值必须为“已启用” ：  

        1.  如果未列出子网，请单击“新建”图标![新建图标](media/Disc_new_Icon.gif)。 在“新建子网分配”  对话框中，输入“子网”  和“掩码”  信息，然后单击“确定” 。 默认情况下，为搜索启用了新子网。  

        2.  要更改所列子网的“搜索”  值，请选择子网，然后单击“切换”  图标以在“已禁用”  和“已启用” 之间切换。  

6.  要将发现配置为搜索域，请单击“域”  选项卡，并在“域”  选项卡上配置以下一个或多个选项：  

    -   要在运行发现的计算机的域上运行发现，请选中“搜索本地域”  复选框。  

    -   为了搜索特定域，该域必须在“域”  中列出，并且其“搜索”  值必须为“已启用” ：  

        1.  如果未列出域，请单击“新建”图标![新建图标](media/Disc_new_Icon.gif)，然后在“域属性”对话框中，输入“域”信息，然后单击“确定”。 默认情况下，为搜索启用了新域。  

        2.  要更改所列域的“搜索”  值，请选择域，然后单击“切换”  图标以在“已禁用”  和“已启用” 之间切换。  

7.  要将发现配置为搜索 SNMP 设备的特定 SNMP 共同体名称，请单击“SNMP”  选项卡，并在“SNMP”  选项卡上配置以下一个或多个选项：  

    -   若要将 SNMP 共同体名称添加到“SNMP 共同体名称”列表，单击“新建”图标![新建图标](media/Disc_new_Icon.gif)，并在“新建 SNMP 共同体名称”对话框中，指定 SNMP 共同体的“名称”，然后单击“确定”。  

    -   若要删除 SNMP 共同体名称，请选择共同体名称，然后单击“删除”图标![删除图标](media/Disc_delete_Icon.gif)。  

    -   若要调整 SNMP 共同体名称的搜索顺序，请选择共同体名称，然后单击“上移项目”图标![上移项目](media/Disc_moveUp_Icon.gif)，或“下移项目”图标![下移项目](media/Disc_moveDown_Icon.gif)。 当运行发现时，会按照从上向下的顺序搜索共同体名称。  
        ...
        -   大多数 SNMP 设备具有默认的“公共”  共同体名称，如果你不知道任何其他共同体名称，则可以使用此名称。 但是，某些组织将删除其设备的“公共”  共同体名称以作为安全措施。  

8.  要配置供 SNMP 搜索使用的最大路由器跃点数，请单击“SNMP”  选项卡，并从 “SNMP”  选项卡上的“最大跃点数”  下拉列表中选择跃点数。  

9. 若要配置“SNMP 设备”，单击“SNMP 设备”选项卡，如果未列出设备，则在“SNMP”选项卡上，单击“新建”图标![新建图标](media/Disc_new_Icon.gif)。 在“新建 SNMP 设备”  对话框中，指定 SNMP 设备的 IP 地址或设备名称，然后单击“确定” 。  

    > [!NOTE]  
    >  如果指定设备名称，则 Configuration Manager 必须能够将 NetBIOS 名称解析为 IP 地址。  

10. 要将发现配置为查询 DHCP 客户端的特定 DHCP 服务器，请单击“DHCP”  选项卡，并在“DHCP”  选项卡上配置以下一个或多个选项：  

    -   要在运行发现的计算机上查询 DHCP 服务器，请选中“始终使用站点服务器的 DHCP 服务器”  复选框。  

        > [!NOTE]  
        >  为了使用此选项，服务器必须从 DHCP 服务器租赁其 IP 地址，并且不能使用静态 IP 地址。  

    -   若要查询特定 DHCP 服务器，请单击“新建”图标![新建图标](media/Disc_new_Icon.gif)，然后在“新建 DHCP 服务器”对话框中，指定 DHCP 服务器的 IP 地址或服务器名称，然后单击“确定”。  

        > [!NOTE]  
        >  如果指定服务器名称，则 Configuration Manager 必须能够将 NetBIOS 名称解析为 IP 地址。  

11. 若要配置发现运行的时间，请单击“计划”选项卡，然后在“计划”选项卡上，单击“新建”图标![新建图标](media/Disc_new_Icon.gif)以设置用于运行网络发现的计划。  

     你可以为网络发现配置多个计划，其中包括多个重复计划以及多个无重复计划。  
...
2.  在“监视”  工作区中，展开“系统状态” ，然后单击“状态消息查询” 。  

3.  选择“所有状态消息” 。  

4.  在“主页”  选项卡上的“状态消息查询”  组中，单击“显示消息” 。  

5.  选择“选择日期和时间”  下拉列表，并选择一个说明多久之前启动发现的值，然后单击“确定”  以打开“Configuration Manager 状态消息查看器” 。  
