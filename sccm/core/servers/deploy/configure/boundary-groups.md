...
- [创建边界组](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_create)  
- [配置边界组](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_config)  
...
关系在边界组属性“关系”  选项卡上配置。在配置关系时，你将定义指向邻居边界组的链接。 对于每种受支持的站点系统角色，可配置独立的设置以回退到该相邻边界组。 有关详细信息，请参阅[配置回退行为](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_bg-fallback)。
...
- [配置站点分配，并选择站点系统服务器](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_references)
- [为自动站点分配配置回退站点](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_site-fallback)
...
- **在操作系统部署期间**，客户端请求用以发送或接收其状态迁移信息的位置。  
    ...
    - 从版本 1810 开始，客户端基于边界组行为获取内容。 有关更多详细信息，请参阅[对边界组的任务序列支持](#bkmk_bgr-osd)。  
...
### <a name="bkmk_ccmsetup"></a> 客户端安装

<!--1358840-->
在安装 Configuration Manager 客户端时，ccmsetup 进程会联系管理点以查找所需的内容。 在版本 1806 及更早版本的此过程中，管理点仅返回客户端当前边界组中的分发点。 如果没有可用内容，安装进程会回退，以从管理点下载内容。 无法回退到其他边界组中可能具有必要内容的分发点。

从版本 1810 开始，管理点会基于边界组配置返回分发点。 如果在边界组中定义了关系，则管理点可按以下顺序返回分发点：
...
<!--1359025-->
从版本 1810 开始，当设备运行任务序列并且需要获取内容时，它现在可以使用类似于 Configuration Manager 客户端的边界组行为。
...
有关如何配置这些设置的详细信息，请参阅[配置边界组](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_config)。

#### <a name="bkmk_bgoptions1"></a> 允许在此边界组中进行对等下载
...
#### <a name="bkmk_bgoptions2"></a> 对等下载期间，只能使用同一子网内的对等设备
...
#### <a name="bkmk_bgoptions3"></a> 优先选择具有相同子网的对等点的分发点

默认情况下，管理点会优先考虑内容位置列表顶部的对等缓存源。 此设置会反转与对等缓存源位于同一子网的客户端的优先级。  

#### <a name="bkmk_bgoptions4"></a> 优先选择分发点上的云分发点
...
有关详细信息，请参阅[将客户端手动切换到新的软件更新点](/sccm/sum/plan-design/plan-for-software-updates#BKMK_ManuallySwitchSUPs)。
...
- 若要使用此选项，请在“层次结构设置”  中启用“客户端首选使用边界组中指定的管理点”  。 然后在各个主站点上配置边界组。 包括应与该边界组的相关边界关联的管理点。 有关详细信息，请参阅[启用对首选管理点的使用](/sccm/core/servers/deploy/configure/boundary-group-procedures#bkmk_proc-prefer)。  
...
- [边界组的过程](/sccm/core/servers/deploy/configure/boundary-group-procedures)  

- [关于边界](/sccm/core/servers/deploy/configure/boundaries)  
