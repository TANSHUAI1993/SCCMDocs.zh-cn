...
-   具有一台安装了 Windows Server 2008（Standard Edition 或 Enterprise Edition、R2 或更高版本）并指定为成员服务器的计算机，并在该计算机上安装了 Internet Information Services (IIS)。 如果必须支持由 Intranet 上的 System Center Configuration 和客户端注册的移动设备，则此计算机将是配置有 Intranet 完全限定的域名 (FQDN) 的 System Center Configuration Manager 站点系统服务器，可以支持 Intranet 和 Internet FQDN 上的客户端连接。  
...
###  <a name="BKMK_AMTprovisioning2008"></a>创建、颁发和安装 AMT 设置证书  
 在通过内部根 CA 的证书指纹设置基于 AMT 的计算机时，使用内部 CA 创建设置证书。 如果不是这种情况，并且必须使用外部证书颁发机构，请使用颁发 AMT 设置证书的公司提供的说明，这些说明通常涉及到从公司的公共网站中申请证书。 还可以在 [Intel vPro Expert Center:Microsoft vPro Manageability 网站](http://go.microsoft.com/fwlink/?LinkId=132001)上找到所选外部 CA 的详细介绍。  
...
10. 在“新建应用程序策略”对话框中，在“名称”字段中输入“AMT 设置”，然后为“对象标识符”输入以下数字：     2.16.840.1.113741.1.2.3  。  

11. 选择“确定”  ，然后选择“添加应用程序策略”  对话框中的“确定”  。  

12. 选择“编辑应用程序策略扩展”  对话框中的“确定”  。  

13. 在“新模板的属性”对话框中，以下内容列为“应用程序策略”说明   ：“服务器身份验证”和“AMT 设置”   。  
