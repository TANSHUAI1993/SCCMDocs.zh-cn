...
System Center Configuration Manager 版本 1610 收集三个级别的诊断和使用情况数据：**基本**、**增强**和**完全**。 默认情况下，此功能设置为增强级别。 以下部分提供在每个级别收集哪些数据的其他详细信息。
...
从 1610 版起，可在控制台中导航至“管理” > “概述” > “站点配置” > “站点”，来更改数据集合级别。 打开“层次结构设置”，然后选择要使用的数据级别。  
...
-   **应用程序管理：**  
    ...
    - ***[更新]*** 适用于企业的 Windows 应用商店 (WSfB) 应用的计数和同步统计信息（包括汇总的应用类型、许可应用的状态、许可应用联机和脱机的数量）  

    - 边界组统计信息（快速和慢速的数量，每个组的计数）

    - MSI 配置选项和计数

    - 应用要求（内置条件计数引用了哪种部署技术）
...
-   **客户端：**  
    ...
    -  ***[更新]*** 客户端自动升级：部署配置包括客户端试点和排除使用情况（扩展的互操作性客户端）
    ...
    - 客户端通知操作的操作状态（每个操作的运行次数，目标客户端的最大数，平均成功率）

    - 用于客户端的部署方法，每个部署方法的客户端计数
...
- **集合：**
    ...
    -  集合评估统计信息（查询时间、已分配与未分配计数、按类型的计数、ID 滚动更新和规则使用情况）
...
-   **移动设备管理 (MDM)：**  
    ...
    -   ***[更新]*** 已发出的移动设备操作（锁定、固定剩余部分、擦除、停用和立即同步）命令的计数  
...
-   MSI 产品代码（客户正在部署的常见应用有哪些）

-   ATP 运行状况摘要

-   详细的客户端部署安装错误

- ***[新增]*** 适用于企业的 Windows 应用商店应用程序详细信息（已同步的应用程序的非聚合列表，包括 AppID、状态 - 联机或脱机，以及购买的许可证总数）
