...
> [!Tip]  
> 此外还需计划管理内容库外部的包源内容。 Configuration Manager 中的每个软件对象都在网络共享上具有包源。 请考虑将所有源集中到单个共享，但请确保此位置冗余且高度可用。 如果将内容库移动到包源所在的同一存储卷，且此卷支持内容的重复数据删除，则此设计可节省必要的存储量。  
