...
|-|-|-|
|日志名称|说明|带有日志文件的计算机|
|CloudMgr.log|记录有关部署云管理网关服务、正在进行的服务状态，以及与服务相关联的使用数据的详细信息。<br>你可以配置日志记录级别编辑注册表 **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\SMS\COMPONENTS\SMS_CLOUD_SERVICES_MANAGER\Logging level**|站点系统服务器上的 **SMS/Logs** 文件夹|
