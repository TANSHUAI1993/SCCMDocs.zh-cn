...
-  **允许内容源位置回退**：这是可以在分发点的“边界组”选项卡上启用的选项。  这使客户端可以使用配置为在内容在首选分发点上不可用时的回退位置的分发点。  
...
3.  将从列表上的回退分发点中下载内容，原因是当客户端使用回退分发点时的部署属性设置为“下载和安装内容” 。 此部署设置允许必须使用回退内容位置的客户端从该位置中获取内容。  
