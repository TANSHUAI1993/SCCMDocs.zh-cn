...
- "**试点设备包含的设备**" 列表包含**目标组**中的设备子集：位于全局试点*包含*列表中的**所有 Windows 10 客户端**：**所有 IT 设备**  
- "**其他建议的设备**" 列表包含**目标组**中的一组设备，这些设备可为重要资产提供最大的覆盖范围和冗余。  桌面分析从此列表中排除全局试验*排除*列表中的任何设备：**首席执行官办公室**
...
4. 输入部署的名称，并选择要使用的任务序列。 使用选项**自动创建默认的二阶段部署**，然后配置以下集合：  
    ...
    - **第一个集合**：查找并选择此部署计划的**试点**集合。 此集合的标准命名约定是 `<deployment plan name> (Pilot)`。
