...
Configuration Manager 支持在运行 Configuration Manager 客户端的 Windows 10 设备上管理适用于企业的 Microsoft Store 的应用，也支持在向 Microsoft Intune 注册的 Windows 10 设备上（称之为混合配置）管理这些应用。 Configuration Manager 对联机和脱机应用提供以下功能。
...
- PC 必须加入了 Azure Active Directory 工作区，并位于将适用于企业的 Microsoft Store 注册为管理工具的 AAD 租户中。
...
- 通过用户安装应用程序，或通过应用程序达到其安装截止时间，或通过对所需部署进行后安装重新评估，强制执行安装时：
    - 应用程序通过启动适用于企业的 Microsoft Store 应用“强制执行”。 
    - 在安装应用前，最终用户必须从应用商店完成安装
    - Configuration Manager 控制台中的应用程序状态会报告失败，并显示错误“Microsoft Store 应用在客户端 PC 上打开并等待用户完成安装。”
- 在下一步的应用程序评估周期中：
...
- 如果部署了禁用 Microsoft Store 的使用者版本的访问权限的组策略，则适用于企业的 Microsoft Store 的部署无法运行，即使已启用适用于企业的 Microsoft Store。
...
4. 完成之后，Configuration Manager 控制台中会出现“适用于企业的 Microsoft Store”列表中配置的帐户。
...
6. 查看“信息”页，并根据指示完成所有额外步骤和配置。 这些配置是通过 Configuration Manager 使用服务所必需的。 例如，配置适用于企业的 Microsoft Store：
    ...
    - 在适用于企业的 Microsoft Store 控制台中，必须将 Configuration Manager 配置为存储管理工具并启用对脱机许可应用的支持，然后购买至少一个应用。 
7. 准备好继续后，单击“下一步”。
...
## <a name="create-and-deploy-a-configuration-manager-application-from-a-microsoft-store-for-business-app"></a>从适用于企业的 Microsoft Store 应用创建和部署 Configuration Manager 应用程序。
...
部署联机应用后，请注意该应用的任何更新将直接来自 Microsoft Store。

使用 Configuration Manager 客户端（尤其是在教室这样的多用户环境中）将脱机应用部署到 Windows 10 设备时，请通过 Microsoft Store（例如，使用[组策略](https://docs.microsoft.com/en-us/windows/configuration/stop-employees-from-using-microsoft-store#a-href-idblock-store-group-policyablock-microsoft-store-using-group-policy)）禁用 Microsoft Store 和/或应用程序更新，使用户无法更新 Configuration Manager 部署外部的应用程序。 此外，在适用于企业的 Microsoft Store 管理员购买应用之后（从而使其在脱机状态下可用于与 Configuration Manager 的同步），请勿将该应用发布给用户，以便他们可联机安装或更新。 这可确保所有用户仅通过 Configuration Manager 接收应用更新。 
