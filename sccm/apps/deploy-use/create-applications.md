...
- **从应用程序属性**：在“应用程序”节点中选择现有的应用程序，然后单击“属性”。 切换到“部署类型”选项卡，然后单击“添加”。
...
2.  在“检测规则”对话框中，单击“设置类型”下拉列表。 选择以下方法之一来检测部署类型的状态：  
    ...
    - **文件系统**：检测设备上是否存在指定的文件或文件夹。 此检测表示已安装应用程序。 指定以下其他详细信息：  
        ...
        - **路径**（必需）：输入或浏览到包含文件或文件夹的设备上的本地路径。 例如，`C:\Program Files`。 无法指定共享网络路径。 如果单击“浏览”，则浏览本地文件系统或连接到代表客户端进行浏览。  
...
指定返回代码可控制部署类型完成后的行为。 例如，通知需要重启、安装完成或自定义向用户显示的文本。 
