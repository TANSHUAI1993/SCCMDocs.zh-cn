...
当前分支版本1806不支持应用程序目录的 Silverlight 用户体验。 此项配置有助于减少向用户交付应用程序所需的服务器基础结构。

从1906版开始, 更新后的客户端自动将管理点用于用户可用的应用程序部署。 还不能安装新的应用程序目录角色。 在 2019 年 10 月 31 日之后的第一个当前分支版本中，应用程序目录角色不再受支持。 减少服务器基础结构还可以减少攻击面。
...
在单独的服务器上安装应用程序目录网站点和应用程序目录 Web 服务点。 如果网站点被泄露, 则与 web 服务点不同。 此设计将有助于保护 Configuration Manager 客户端和基础结构。 如果网站点接受来自 Internet 的客户端连接，则此配置非常重要。 它使服务器更易受攻击。  
...
在“计算机代理”  组中配置“安装权限”  客户端设置。 此设置限制可以在软件中心安装软件的用户的类型。
...
- 安装 Configuration Manager 客户端时，还会在设备上自动安装软件中心  。 用户更改设置, 从软件中心浏览并安装软件。  
...
### <a name="bkmk_privacy-uda"></a>用户设备相关性隐私信息
...
### <a name="bkmk_privacy-userex"></a>软件中心隐私信息
